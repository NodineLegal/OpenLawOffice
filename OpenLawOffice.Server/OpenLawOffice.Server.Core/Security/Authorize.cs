            ...
            Rest.Requests.Security.Area areaRequest, Common.Models.PermissionType permissionRequired)
        {
            ...
            using (IDbConnection db = Database.Instance.OpenConnection())
            {
                ...
                if (!areaRequest.Id.HasValue)
                {
                    ...
                    dbArea = db.Single<DBOs.Security.Area>(new { areaRequest.Name });
                }
                else
                {
                ...
                if (!userRequest.Id.HasValue)
                {
                    ...
                    {
                        // Fetch by authtoken
                        dbUser = db.Single<DBOs.Security.User>(new { UserAuthToken = userRequest.AuthToken });
                    }
                    else if (!string.IsNullOrEmpty(userRequest.Username))
                    {
                        // We know from validation that we must now have a Username
                        dbUser = db.Single<DBOs.Security.User>(new { userRequest.Username });
                    }
                    else
                        return new AuthorizeResult()
                        {
                ...
                dbAreaAcl = db.SqlListSingle<DBOs.Security.AreaAcl>(new { SecurityAreaId = dbArea.Id, UserId = dbUser.Id });
            }
            ...
            Rest.Requests.Security.SecuredResource securedResourceRequest, Common.Models.PermissionType permissionRequired)
        {
            ...
            using (IDbConnection db = Database.Instance.OpenConnection())
            {
                ...
                if (!userRequest.Id.HasValue)
                {
                    ...
                    {
                        // Fetch by authtoken
                        dbUser = db.SqlListSingle<DBOs.Security.User>(new { UserAuthToken = userRequest.AuthToken });
                    }
                    else
                    {
                        // We know from validation that we must now have a Username
                        dbUser = db.SqlListSingle<DBOs.Security.User>(new { Username = userRequest.Username });
                    }
                }
                else
                {
                ...
                dbSecuredResourceAcl = db.SqlListSingle<DBOs.Security.SecuredResourceAcl>(
