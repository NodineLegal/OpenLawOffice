namespace OpenLawOffice.Data.Mappings.Security
{
    ...
    [Common.Models.MapMe]
    internal class SecuredResourceAcl
    {
        internal void BuildMappings()
        {
            Mapper.CreateMap<DbModels.SecuredResourceAcl, Common.Models.Security.SecuredResourceAcl>()
                ...
                .ForMember(dst => dst.SecuredResource, opt => opt.ResolveUsing(db =>
                {
                    ...
                    {
                        Id = Guid.Parse(db.SecuredResourceId),
                        IsStub = true
                    };
                }))
            ...
            Mapper.CreateMap<Common.Models.Security.SecuredResourceAcl, DbModels.SecuredResourceAcl>()
                .ForMember(dst => dst.UtcCreated, opt => opt.MapFrom(src => src.UtcCreated))
                .ForMember(dst => dst.UtcModified, opt => opt.MapFrom(src => src.UtcModified))
                .ForMember(dst => dst.UtcDisabled, opt => opt.MapFrom(src => src.UtcDisabled))
